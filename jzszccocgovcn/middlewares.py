...
class JzszccocgovcnSpiderMiddleware(object):
...
class JzszccocgovcnDownloaderMiddleware(object):
    # Not all methods need to be defined. If a method is not defined,
...
class ProxyMiddleware(object):
    ...
    def process_request(self, request, spider):
        print("当前访问连接：", request.url)

        # proxyAuth = "Basic " + base64.urlsafe_b64encode(bytes((self.proxyUser + ":" + self.proxyPass), "ascii")).decode(
        #     "utf8")
        # request.meta["proxy"] = self.proxyServer
        # request.headers["Proxy-Authorization"] = proxyAuth
        if request.meta.get("path"):
            request.headers["Cookie"] = f"JSESSIONID={self.cookies.get('JSESSIONID')};"
